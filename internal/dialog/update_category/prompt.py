...
1. КОНФЛИКТЫ между good_samples и bad_samples:
   - Сравни содержимое/темы/правила из good и bad
   - Если есть противоречия (например: good говорит "используй эмодзи", bad говорит "избегай эмодзи") → зафиксируй конфликт

2. ДУБЛИКАТЫ в good_samples:
   - Проверь похожие паттерны/правила
   - Если несколько samples учат одному и тому же → зафиксируй дубль

3. ДУБЛИКАТЫ в bad_samples:
   - Проверь похожие антипаттерны
   - Если несколько samples предупреждают об одном → зафиксируй дубль
...
<blockquote>
[Перечисли конфликты, например:]
• Хорошие примеры говорят "используй эмодзи", но плохие примеры говорят "избегай эмодзи"<br>
• Хорошие примеры содержат длинные тексты, но плохие примеры критикуют за многословие
</blockquote>

[ЕСЛИ ЕСТЬ ДУБЛИ В GOOD_SAMPLES:]
<p><b>Похожие паттерны (дубли):</b></p>
<blockquote>
[Перечисли группы похожих паттернов, например:]
• 3 примера учат одному и тому же: "добавляй призыв к действию в конце"<br>
• 2 примера дублируют правило про эмодзи
</blockquote>

[ЕСЛИ ЕСТЬ ДУБЛИ В BAD_SAMPLES:]
<p><b>Повторяющиеся антипаттерны:</b></p>
<blockquote>
[Перечисли группы похожих антипаттернов]
</blockquote>
...
<ol>
<li><b>Добавить Telegram каналы</b> — система проанализирует их и предложит изменения</li>
...
- next_options:
  ...
  * По умолчанию предложи начать с добавления каналов (stage 2)
</transition>
