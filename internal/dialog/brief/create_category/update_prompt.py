...
2. Следуй единой структуре сообщений на каждом этапе
...
<blockquote>
<b>Примеры тем для рубрики "{category.name}":</b>
- [конкретный пример темы 1, подходящий для этой рубрики]
- [конкретный пример темы 2, подходящий для этой рубрики]
</blockquote>
...
2. ПОТОМ: Запрос мнения пользователя
...
<details>
<summary>Посмотреть плюсы</summary>
<ol>
...
<details open>
<summary>Посмотреть замечания</summary>
<ol>
...
<details>
<summary>Как сделать лучше</summary>
...
<blockquote>
<span><b>✅ Эталонных постов:</b> {{full_examples_count}}</span>
<span><b>📝 Паттернов успеха:</b> {{количество паттернов в accumulated_good_samples}}</span>
<span><b>❌ Правил что избегать:</b> {{количество антипаттернов в accumulated_bad_samples}}</span>
</blockquote>
...
<details open>
<summary>Паттерны успеха</summary>
<blockquote>
...
2. ⚠️ В stage 1.3 ОБЯЗАТЕЛЬНО: сначала твоя критика → потом запрос фидбека
...
<span><b>Сохраняем рубрику?</b></span>
</message_template>
...
4. После подтверждения - верни category_data со ВСЕМИ данными
</processing>
...
{{
  "message_to_user": "✅ Сохраняю рубрику...",
  "current_stage": "2",
  "prev_stage": "2",
  "next_stage": null,
...
3. СНАЧАЛА твоя критика, ПОТОМ запрос фидбека (1.3)
