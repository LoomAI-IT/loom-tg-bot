...
class DraftPublicationDialog(interface.IDraftPublicationDialog):

    def __init__(
            self,
            tel: interface.ITelemetry,
            draft_publication_service: interface.IDraftPublicationService,
            draft_publication_getter: interface.IDraftPublicationGetter,
    ):
        self.tracer = tel.tracer()
        self.logger = tel.logger()
        self.draft_publication_service = draft_publication_service
        self.draft_publication_getter = draft_publication_getter
        ...
        return Window(
                ...
                Case(
                    {
                        ...
                        False: Multi(
                            Const("✅ <b>Нет черновиков</b><br><br>"),
            ...
            Row(
                Button(
                    Const("⬅️ Пред"),
                    id="prev_publication",
                    on_click=self.draft_publication_service.handle_navigate_publication,
                    when="has_prev",
                ),
                Button(
        ...
        return Window(
            ...
            Row(
                Button(
                    ...
                    id="prev_image",
                    on_click=self.draft_publication_service.handle_prev_image,
                    when="has_multiple_images",
                ),
                Button(
                    Const("➡️ След изображение"),
                    id="next_image",
                    on_click=self.draft_publication_service.handle_next_image,
                    when="has_multiple_images",
                ),
                when="has_multiple_images",
            ),
            ...
            Column(
                Button(
                    ...
                    id="edit_text_menu",
                    on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.edit_text_menu),
                ),
                Button(
                    Const("🖼️ Изменить изображение"),
                    id="edit_image",
                    on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.edit_image_menu),
                ),
            ),
            ...
            Row(
                Button(
                    ...
                    id="save_edits",
                    on_click=self.draft_publication_service.handle_save_edits,
                    when="has_changes",
                ),
                Button(
                    Const("◀️ Назад к списку"),
                    id="back_to_draft_list",
                    on_click=self.draft_publication_service.handle_back_to_draft_list,
                ),
            ),

            state=model.DraftPublicationStates.edit_preview,
            getter=self.draft_publication_getter.get_edit_preview_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
    ...
    def get_edit_text_menu_window(self) -> Window:
        return Window(
            ...
            MessageInput(
                func=self.draft_publication_service.handle_regenerate_text_prompt_input,
            ),
            ...
            Column(
                Button(
                    ...
                    id="regenerate_all",
                    on_click=self.draft_publication_service.handle_regenerate_text,
                    when=~F["is_regenerating_text"]
                ),
                Button(
                    Const("✍️ Написать свой текст"),
                    id="edit_content",
                    on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.edit_text, ShowMode.EDIT),
                    when=~F["is_regenerating_text"]
                ),
            ),
            Button(
            ...
            state=model.DraftPublicationStates.edit_text_menu,
            getter=self.draft_publication_getter.get_edit_publication_text_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
    ...
    def get_edit_text_window(self) -> Window:
        return Window(
            ...
            TextInput(
                id="text_input",
                on_success=self.draft_publication_service.handle_edit_publication_text,
            ),

            Button(
                Const("◀️ Назад"),
                id="edit_text_menu",
                on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.edit_text_menu, ShowMode.EDIT),
            ),

            state=model.DraftPublicationStates.edit_text,
            getter=self.draft_publication_getter.get_edit_publication_text_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
    ...
    def get_edit_image_menu_window(self) -> Window:
        return Window(
            ...
            MessageInput(
                func=self.draft_publication_service.handle_edit_image_prompt_input,
            ),
            ...
            Column(
                Button(
                    ...
                    id="generate_image",
                    on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.image_generation_mode_select,
                                                         ShowMode.SEND),
                ),
                Button(
                    Const("🖇 Внести правки в изображение"),
                    id="edit_image",
                    on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.edit_image_input, ShowMode.EDIT),
                    when="has_image",
                ),
                Button(
                    Const("📷 Использовать своё фото"),
                    id="upload_image",
                    on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.upload_image, ShowMode.EDIT),
                ),
                Button(
                    Const("📐 Объединить изображения"),
                    id="combine_images",
                    on_click=self.draft_publication_service.handle_combine_images_start,
                ),
                Button(
                    Const("🗑️ Удалить изображение"),
                    id="remove_image",
                    on_click=self.draft_publication_service.handle_remove_image,
                    when="has_image",
                ),
                when=~F["is_generating_image"]
            ),
            ...
            state=model.DraftPublicationStates.edit_image_menu,
            getter=self.draft_publication_getter.get_image_menu_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
    ...
    def get_upload_image_window(self) -> Window:
        return Window(
            ...
            MessageInput(
                func=self.draft_publication_service.handle_image_upload,
            ),

            Button(
                Const("◀️ Назад"),
                id="image_menu",
                on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.edit_image_menu, ShowMode.EDIT),
            ),

            state=model.DraftPublicationStates.upload_image,
            getter=self.draft_publication_getter.get_upload_image_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
    ...
    def get_edit_image_input_window(self) -> Window:
        return Window(
            ...
            state=model.DraftPublicationStates.edit_image_input,
            getter=self.draft_publication_getter.get_edit_image_input_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
        ...
        return Window(
            ...
            Column(
                Button(
                    ...
                    id="auto_generate",
                    on_click=self.draft_publication_service.handle_auto_generate_image,
                ),
                Button(
                    Const("🖍 По моему запросу"),
                    id="custom_generate",
                    on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.reference_image_generation,
                                                         ShowMode.EDIT),
                ),
            ),
            ...
            state=model.DraftPublicationStates.image_generation_mode_select,
            getter=self.draft_publication_getter.get_image_generation_mode_select_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
    ...
    def get_reference_generation_image_window(self) -> Window:
        return Window(
            ...
            MessageInput(
                func=self.draft_publication_service.handle_reference_generation_image_prompt_input,
            ),
            ...
            Column(
                Button(
                    ...
                    on_click=self.draft_publication_service.handle_use_current_image_as_reference,
                    when=F["has_image"] & ~F["has_reference_generation_image"]
                ),
                Button(
                    Const("🌅 Добавить своё фото"),
                    id="add_custom_photo",
                    on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.reference_image_upload,
                                                         ShowMode.EDIT),
                ...
                Button(
                    ...
                    on_click=self.draft_publication_service.handle_remove_reference_generation_image,
                    when=F["has_reference_generation_image"]
                ),
                when=~F["is_generating_image"]
            ),
            ...
            Button(
                ...
                on_click=self.draft_publication_service.handle_back_from_custom_generation,
                when=~F["is_generating_image"]
            ),

            state=model.DraftPublicationStates.reference_image_generation,
            getter=self.draft_publication_getter.get_reference_generation_image_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
    ...
    def get_reference_generation_image_upload_window(self) -> Window:
        return Window(
            ...
            MessageInput(
                func=self.draft_publication_service.handle_reference_generation_image_upload,
            ),
            ...
            state=model.DraftPublicationStates.reference_image_upload,
            getter=self.draft_publication_getter.get_reference_generation_image_upload_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
        ...
        return Window(
            ...
            Column(
                Button(
                    Const("📝 Сжать текст"),
                    id="compress_text",
                    on_click=self.draft_publication_service.handle_compress_text,
                ),
                Button(
                    Const("🗑️ Отказаться от фото"),
                    id="remove_photo",
                    on_click=self.draft_publication_service.handle_remove_photo_from_long_text,
                ),
                Button(
                    Const("↩️ Вернуть предыдущий текст"),
                    id="restore_previous",
                    on_click=self.draft_publication_service.handle_restore_previous_state,
                    when="has_previous_text",
                ),
            ),

            state=model.DraftPublicationStates.text_too_long_alert,
            getter=self.draft_publication_getter.get_text_too_long_alert_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
        ...
        return Window(
            ...
            Row(
                Button(
                    Const("⬅️ Старая"),
                    id="show_old_image",
                    on_click=self.draft_publication_service.handle_show_old_image,
                    when=F["has_old_image"] & F["showing_new_image"] & ~F["is_applying_edits"]
                ),
                Button(
                    Const("➡️ Новая"),
                    id="show_new_image",
                    on_click=self.draft_publication_service.handle_show_new_image,
            ...
            MessageInput(
                func=self.draft_publication_service.handle_edit_image_prompt_input_from_confirm_new_image,
            ),
            ...
            Column(
                Button(
                    ...
                    on_click=self.draft_publication_service.handle_combine_image_from_new_image,
                ...
                Row(
                    Button(
                        ...
                        on_click=self.draft_publication_service.handle_confirm_new_image,
                        when=~F["is_applying_edits"]
                    ),
                    Button(
                        Const("❌ Отклонить"),
                        id="reject_new_image",
                        on_click=self.draft_publication_service.handle_reject_new_image,
                        when=~F["is_applying_edits"]
                    ),
                ),
            ),

            state=model.DraftPublicationStates.new_image_confirm,
            getter=self.draft_publication_getter.get_new_image_confirm_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
        ...
        return Window(
            ...
            Column(
                Button(
                    ...
                    on_click=self.draft_publication_service.handle_combine_with_current_image,
                    when="has_image",
                ),
                Button(
                    Const("🔄 Начать с новых"),
                    id="combine_from_scratch",
                    on_click=self.draft_publication_service.handle_combine_image_from_scratch,
                    when="has_image",
                ),
            ),
            ...
            state=model.DraftPublicationStates.combine_images_choice,
            getter=self.draft_publication_getter.get_combine_images_choice_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
        ...
        return Window(
            ...
            Row(
                Button(
                    ...
                    on_click=self.draft_publication_service.handle_prev_combine_image,
                    when="has_multiple_combine_images",
                ),
                Button(
                    Const("➡️ Следующая"),
                    id="next_combine_image",
                    on_click=self.draft_publication_service.handle_next_combine_image,
                    when="has_multiple_combine_images",
                ),
                when="has_multiple_combine_images",
            ),
            ...
            MessageInput(
                func=self.draft_publication_service.handle_combine_image_upload,
            ),
            ...
            Column(
                Button(
                    ...
                    on_click=self.draft_publication_service.handle_delete_combine_image,
                    when="has_combine_images",
                ),
                Button(
                    Const("▶️ Далее"),
            ...
            Button(
                ...
                on_click=self.draft_publication_service.handle_back_from_combine_image_upload,
            ),

            state=model.DraftPublicationStates.combine_images_upload,
            getter=self.draft_publication_getter.get_combine_images_upload_data,
            parse_mode=SULGUK_PARSE_MODE,
        )
    ...
    def get_combine_images_prompt_window(self) -> Window:
        return Window(
            ...
            MessageInput(
                func=self.draft_publication_service.handle_combine_image_prompt_input,
            ),

            Button(
                Const("⏭️ Пропустить"),
                id="skip_prompt",
                on_click=self.draft_publication_service.handle_skip_combine_image_prompt,
                when=~F["is_combining_images"]
            ),
            ...
            Button(
                Const("◀️ Назад"),
                id="back_to_upload",
                on_click=lambda c, b, d: d.switch_to(model.DraftPublicationStates.combine_images_upload, ShowMode.EDIT),
                when=~F["is_combining_images"]
            ),
